...
interface InventoryProduct {
  ...
  weight_options?: Array<{ weight_oz: number; label: string; in_stock: boolean }> | null;
...
export default function InventoryPage() {
  ...
  const fetchProducts = useCallback(async (sid: string) => {
    try {
      ...
      const res = await fetch(
        `${SUPABASE_URL}/rest/v1/products?seller_id=eq.${sid}&select=id,name,price,stock_qty,unit,in_stock,status,images,pricing_type,price_per_pound,weight_options,low_stock_threshold,is_seasonal,restock_notes,categories(name)&order=name.asc`,
        { headers }
      );
      if (!res.ok) return;
  ...
  // Load restock history from localStorage (simple local log)
  useEffect(() => {
    try {
  ...
  // When a product is selected, pre-fill settings panel
  useEffect(() => {
  ...
  const handleRestock = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        alert("Error restocking: " + await res.text());
        return;
      }
      ...
      // Refresh products & update selection
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        alert("Error saving settings: " + await res.text());
        return;
      }
      ...
      <div className="space-y-5 max-w-6xl">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Manage stock levels and product settings</p>
          </div>
        </div>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {filtered.map(p => {
                        ...
                        return (
                          <tr key={p.id} className={`transition-colors ${isSelected ? "bg-green-50/60" : "hover:bg-gray-50/50"}`}>
                            {/* Thumbnail */}
                            ...
                            <td className="px-4 py-3 whitespace-nowrap">
                              {p.pricing_type === "per_pound" ? (
                                <span className="text-sm">
                                  <span className="font-semibold text-[#1a4a2e]">${Number(p.price_per_pound ?? 0).toFixed(2)}</span>
                            ...
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-1.5">
                                <button
                                  onClick={() => handleInlineStockUpdate(p.id, Math.max(0, (p.stock_qty ?? 0) - 1))}
                                  className="w-6 h-6 rounded-full bg-gray-100 hover:bg-gray-200 border border-gray-200 flex items-center justify-center text-gray-500 text-sm font-bold leading-none transition-colors">
                                  −
                                </button>
                                <input
                                  type="number"
                                ...
                                />
                                <button
                                  onClick={() => handleInlineStockUpdate(p.id, (p.stock_qty ?? 0) + 1)}
                                  className="w-6 h-6 rounded-full bg-gray-100 hover:bg-gray-200 border border-gray-200 flex items-center justify-center text-gray-500 text-sm font-bold leading-none transition-colors">
                                  +
                                </button>
                              ...
                              <button
                                ...
                                className={`text-xs font-semibold px-3 py-1.5 rounded-lg border transition-colors ${
                                  isSelected
                                    ...
                                    : "text-[#1a4a2e] border-[#1a4a2e] hover:bg-[#1a4a2e]/5"
                                }`}>
            ...
            <div className="w-72 shrink-0 space-y-4">
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                ...
                <div className="bg-gray-50 rounded-xl p-3 mb-4 text-center">
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">Current Stock {selectedProduct.unit ? `(${selectedProduct.unit})` : ""}</p>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <input
                      type="text" placeholder="e.g. Harvested today"
                      value={restockNote}
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">Mark as available only during certain seasons</p>
                    </div>
                    <button
                      type="button"
                      onClick={() => setIsSeasonal(v => !v)}
                      className={`relative w-10 h-5.5 rounded-full transition-colors ${isSeasonal ? "bg-[#1a4a2e]" : "bg-gray-300"}`}
                  ...
                  <div>
                    ...
                    <textarea
                      rows={3}
                      placeholder="e.g. Harvest every Tuesday. Contact supplier for bulk orders."
                      value={restockNotes}
                  ...
                  {/* Weight options overview (per_pound only) */}
                  {selectedProduct.pricing_type === "per_pound" && selectedProduct.weight_options?.length ? (
                    <div>
                      <p className="text-xs font-medium text-gray-700 mb-2">Weight Options</p>
                      <div className="space-y-1.5">
                        {selectedProduct.weight_options.map((opt, i) => (
                          <div key={i} className="flex items-center justify-between bg-gray-50 rounded-lg px-3 py-2">
                            <span className="text-xs font-medium text-gray-700">{opt.label}</span>
                            <span className={`text-xs font-semibold ${opt.in_stock ? "text-green-600" : "text-red-500"}`}>
                              {opt.in_stock ? "In Stock" : "Out"}
                            </span>
                          </div>
                        ))}
                      </div>
                      <p className="text-xs text-gray-400 mt-1.5">Edit weight options in the Products page.</p>
                    </div>
                  ) : null}
