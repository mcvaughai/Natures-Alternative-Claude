-- ============================================================
-- Nature's Alternative Marketplace — Schema (Safe / Idempotent)
-- Safe to re-run: uses IF NOT EXISTS on tables, DROP POLICY IF
-- EXISTS before every policy, ON CONFLICT DO NOTHING on inserts.
-- ============================================================
...
DROP POLICY IF EXISTS "sellers_select_approved" ON public.sellers;
CREATE POLICY "sellers_select_approved" ON public.sellers
...
DROP POLICY IF EXISTS "applications_insert_any" ON public.seller_applications;
CREATE POLICY "applications_insert_any" ON public.seller_applications
  FOR INSERT WITH CHECK (true);

DROP POLICY IF EXISTS "applications_select_own" ON public.seller_applications;
CREATE POLICY "applications_select_own" ON public.seller_applications
...
CREATE POLICY "products_delete_own_seller" ON public.products
  FOR DELETE USING (
    EXISTS (SELECT 1 FROM public.sellers WHERE id = seller_id AND user_id = auth.uid())
  );
...
DROP POLICY IF EXISTS "auto_harvest_lists_own" ON public.auto_harvest_lists;
CREATE POLICY "auto_harvest_lists_own" ON public.auto_harvest_lists
  USING (auth.uid() = user_id);
...
DROP POLICY IF EXISTS "auto_harvest_items_own" ON public.auto_harvest_items;
CREATE POLICY "auto_harvest_items_own" ON public.auto_harvest_items
  USING (
...
DROP POLICY IF EXISTS "payouts_select_own_seller" ON public.payouts;
